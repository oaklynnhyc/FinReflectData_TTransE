...
try:
    ...
    # context can only be set once per process; ignore if already set
    pass
...
if __name__ == "__main__":
    ...
    if args.test == 0:
        ...
        for epoch in range(config.train_times):
            ...
            if epoch % 5 == 0:
                ...
                print("Train total loss: %d %f" % (epoch, total_loss.item()))
    ...
    # ---- pretty-print to stdout so the user can see it on screen ----
    print()
    print('=' * 60)
    print('  TEST SET EVALUATION  (dataset = %s)' % args.dataset)
    print('=' * 60)
    print('  Hits@1   : %.4f  (%.2f%%)' % (hit1Test, 100 * hit1Test))
    print('  Hits@3   : %.4f  (%.2f%%)' % (hit3Test, 100 * hit3Test))
    print('  Hits@10  : %.4f  (%.2f%%)' % (hit10Test, 100 * hit10Test))
    print('  MeanRank : %.2f   (lower is better)' % meanrankTest)
    print('  MRR      : %.4f   (higher is better)' % meanrerankTest)
    print('=' * 60)
    # ------------------------------------------------------------------
    ...
    print('[saved] ./result/%s.txt  (one row appended)' % args.dataset)
